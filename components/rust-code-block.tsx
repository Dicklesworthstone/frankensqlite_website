...
function tokenizeCodeSegment(segment: string): Token[] {
  ...
  while ((m = CODE_TOKEN_RE.exec(segment)) !== null) {
    if (m.index > lastIndex)
      tokens.push({ kind: "plain", text: segment.slice(lastIndex, m.index) });
    const text = m[0];
    if (text === "::") tokens.push({ kind: "path", text });
    else if (text.endsWith("!"))
      tokens.push({ kind: MACROS.has(text.slice(0, -1)) ? "macro" : "plain", text });
...
function tokenClass(kind: TokenKind): string {
  switch (kind) {
    ...
    case "keyword":
      return "text-teal-400 font-black";
    case "type":
      return "text-teal-300 font-bold";
    case "macro":
        ...
        >
          {copied ? (
            <>
              ...
              <span className="text-teal-400">ARCHIVED</span>
            </>
          ) : (
            <>
        ...
        <AnimatePresence>
          {copied && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-teal-500 z-20 pointer-events-none"
            />
          )}
        </AnimatePresence>
          ...
          <code>
            {tokenLines.map((lineTokens, i) => (
              <span key={i} className="flex group/line">
                <span className="mr-8 inline-block w-6 select-none text-right text-[10px] font-black text-slate-800 group-hover/line:text-teal-500/40 transition-colors">
